...
# Runs daily at 00:05 UTC. For every workspace with active subscription:
...
def dbwcare_monthly_balance_init():
    ...
    """
    from plane.db.models import WorkspaceCareSubscription, WorkspaceMonthlyBalance
    from plane.app.views.care.balance import get_or_create_current_balance

    now = timezone.now()
    year, month = now.year, now.month

    active_subs = WorkspaceCareSubscription.objects.filter(is_active=True)

    for sub in active_subs:
        try:
            get_or_create_current_balance(sub.workspace_id)
        except Exception as e:
            logger.error(
                f"Failed to init balance for workspace {sub.workspace_id}: {e}"
            )
    ...
    logger.info(f"DBWCARE balance init completed for {active_subs.count()} workspaces")
